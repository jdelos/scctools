classdef SCC_node < handle
    ...
    methods
        function SC = SCC(inc_caps,varargin)
        
        %Initialize Capacitor incidence matrix
        SC.inc_caps = inc_caps;  %Capacitors
        SC.n_caps = size(inc_caps,2);
        
        %Number of loads
        SC.n_outs = size(inc_caps,1)-1;
       ...
       while i<=(nargin-1)
           ...
           switch varargin{i}
               ...
               case 'Io'
                   ...
                   try
                     if length(varargin{i+1})==1  
                        SC.current_vector = ones(1,SC.n_outs)*varargin{i+1};
                     else
               ...
               case 'Ro'
                   ...
                   try
                     if length(varargin{i+1})==1 
                        SC.load_resistances(1,1:SC.n_outs) = varargin{i+1};
                     else
       ...
       %Create Loads incidence matrix
            ...
            if SC.n_phases >0  
              ...
              sw_idx=1;
                for i=ph_lst %Switches matrixs
                  ...
                  idx(i)=[];              
                    if i==ph_lst(end)
                        SC.duty(i)=1-sum(SC.duty);
                    else
                        SC.duty(i)=sym(['D' num2str(i)]);
                    end
                    
                    %Initialize phase 
                    SC.phase{i}=SCC_Phase(varargin{i},...
                    inc_caps,varargin{idx},...
                    SC.inc_loads*SC.duty(i),...
                    SC.n_caps,...
                    sw_idx,...
                    SC.in_node);
        ...
        function gen_k_ssl(SC)
            ...
            SC.k_ssl = 1/(2*SC.fsw_op)*...
                           D3_matrix_addition(r_tot,1./SC.caps);
        end
