...
public:
    GenericSwitchedCapacitor(const map_basic_basic& ArchDesc, const std::vector<std::string>& varargin = {}) {
        ...
        if (ArchDesc.find(symbol("Acaps")) != ArchDesc.end()) {
            inc_caps = rcp_static_cast<const DenseMatrix>(ArchDesc.at(symbol("Acaps")));
        } else {
        ...
        if (ArchDesc.find(symbol("Asw")) != ArchDesc.end()) {
            inc_switches = rcp_static_cast<const DenseMatrix>(ArchDesc.at(symbol("Asw")));
        } else {
        ...
        if (ArchDesc.find(symbol("Asw_act")) != ArchDesc.end()) {
            sw_activation_matrix = rcp_static_cast<const DenseMatrix>(ArchDesc.at(symbol("Asw_act")));
        } else {
